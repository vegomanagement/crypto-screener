...
# ─── Этап 12 фаза 2: Top-down PDA bias gate (ICT) ─────────────────────────
# PDF SMC: «If HTF is bullish but LTF is bearish, expect price to move into
# HTF discount» — HTF диктует. compute_htf_bias агрегирует zone-голосование
# по 1H+4H+D. Strong (все доступные TF единогласны и n>=2) → hard WAIT, если
# signal direction противоречит HTF. Moderate/weak/neutral — гейт молчит
# (но bias всё равно пишется в decision dict для LLM/диагностики).
#
...
def make_decision(
    ...
    # P4 (Этап 12 фаза 2): HTF Top-down PDA bias gate.
    gate_reason = _htf_pda_bias_gate(direction, market, base)
    if gate_reason:
        base["verdict"] = "WAIT"
        base["reason"]  = gate_reason
        _strip_levels(base)
...
def _htf_pda_bias_gate(direction: str, market: dict,
                       base: dict) -> str | None:
    """
    Этап 12 фаза 2: жёсткий WAIT, если direction сигнала противоречит
    strong HTF bias (все доступные HTF в одну сторону, минимум 2 TF).

    Идея ICT: HTF диктует драй цены (zone PREMIUM/DISCOUNT). Сигнал против
    strong HTF — высокий риск; пропускаем.

    base['htf_bias'] заполняется всегда (для LLM/диагностики), даже когда
    гейт молчит. Graceful fallback: нет HTF klines → bias neutral → молчим.
    """
    if direction not in ("long", "short"):
        return None
    if not HTF_BIAS_GATE_ENABLED:
        return None
    ...
    base["htf_bias"] = {
        ...
        "zones":         bias.zones,
        "votes_long":    bias.votes_long,
        "votes_short":   bias.votes_short,
        "available_tfs": bias.available_tfs,
    }

    # Hard-gate только при strong против direction.
    if bias.strength != "strong":
        return None
    if bias.direction == direction:
        return None

    return (f"HTF strong bias '{bias.direction}' (zones: {bias.zones}) "
            f"против сигнала {direction} — жёсткий WAIT "
            f"(Этап 12 P4 HTF PDA gate)")
